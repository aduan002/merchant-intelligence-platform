...
**Example Insight:**  
Customer activity is primarily driven by new customers, with returning customers forming a noticeably smaller share across all months. This indicates a high acquisition but low repeat purchase rate. The orders per customer distribution is heavily right-skewed, showing that most customers place only one order, while a small subset palces multiple orders. Revenue by state is highly concentrated, with São Paulo (SP) generating the majority of revenue, followed by Rio de Janeiro (RJ) and Minas Gerais (MG). The cohort matrix shows strong initial acquisition in mid-year months (May–August) but sharp decline in subsequent periods. This highlights retention as a key growth opportunity. The geographic map confirms that customer demand is heavily concentrated in Brazil’s southeast region.
...
**Example Insight:**  
Average delivery times are longest in Roraima, Amapá, and Amazonas, exceeding 25–30 days, indicating significant logistical challenges in northern regions. However, several high volume seller states such as São Paulo, Rio de Janeiro, and Paraná still exhibit below average on-time delivery rates. This means that scale alone does not guarantee delivery efficiency. Cancellation rates peak in Goiás and Minas Gerais, while southern states such as Rio Grande do Sul and Pernambuco show the lowest cancellation levels. The comparison between delivery status and customer reviews reveals a strong service quality relationship, where on-time deliveries achieve average review scores above 4, while late deliveries fall to slightly above 2. It is clear that logistics performance has a direct impact on customer satisfaction. These patterns highlight the need for logistics process improvements and more accurate delivery time estimates to better manage customer expectations and improve overall service quality.
...
**Example Insight:**  
The platform is heavily dominated by credit card payments, which account for approximately 75% of all orders, followed by boleto (~20%), while voucher and debit card usage remain minimal. Payment method preferences remain stable across all months, indicating consistent customer behavior over time. Credit card and debit card payments exhibit the highest cancellation rates, while vouchers show the lowest, implying that prepaid payment methods are associated with lower cancellation risk. The installments vs. average order value analysis shows a clear positive relationship, where higher-value purchases are increasingly split across more installments. Through this, we can confirm that installment plans play a critical role in enabling larger transactions. There is a strong opportunity to expand and optimize installment-based payment options to further enable higher-value purchases, while also strengthening credit risk controls to mitigate the higher cancellation exposure associated with card-based payments.
...
cd merchant-intelligence-platform
```
